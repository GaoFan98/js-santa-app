...
describe('Validation Service', () => {
  ...
  it('should validate a registered child under 10 years old', async () => {
    const result = await validationService.validateChild('730b0412-72c7-11e9-a923-1681be663d3e');
  ...
  it('should return an error for a child not registered', async () => {
    const result = await validationService.validateChild('invalid-id');
    expect(result.isValid).toBe(false);
    expect(result.error).toBe('Child not registered.');
  });

  it('should return an error for a child older than 10 years', async () => {
    const result = await validationService.validateChild('730b06a6-72c7-11e9-a923-1681be663d3e');
    expect(result.isValid).toBe(false);
